...
def show_main_dashboard():
    ...
    try:
                    ...
                    try:
                        ...
                        if analysis:
                            ...
                            def generate_justification(market_type, option, option_name, real_prob, implied_prob, probabilities, home_team, away_team):
                                ...
                                # Justificativas específicas para cada tipo de mercado
                                if market_type == "moneyline":
                                    if option == "home_win":
                                        return f"O time {home_team} tem mostrado desempenho superior em casa que não está refletido nas odds."
                                    elif option == "away_win":
                                        return f"O time {away_team} está em boa forma e as odds subestimam seu potencial contra este adversário."
                                    elif option == "draw":
                                        return "As estatísticas indicam maior probabilidade de empate do que as odds sugerem."
                                
                                elif market_type == "double_chance":
                                    return f"O valor calculado de {real_prob:.1f}% para {option_name} oferece margem significativa de {(real_prob-implied_prob):.1f}% contra a probabilidade implícita."
                                
                                elif market_type == "btts":
                                    if option == "yes":
                                        return "Ambas equipes têm bom retrospecto ofensivo e defesas vulneráveis."
                                    else:
                                        return "O histórico defensivo sólido e/ou ataque ineficiente sugerem baixa probabilidade de ambos marcarem."
                                
                                elif market_type == "over_under":
                                    if "over" in option:
                                        return "O volume de gols esperado com base nas estatísticas é maior do que a linha estabelecida pelas casas de apostas."
                                    else:
                                        return "O perfil defensivo das equipes sugere uma partida com menos gols do que a linha estabelecida."
                                
                                elif market_type == "corners":
                                    if "over" in option:
                                        return "O estilo de jogo de ambas equipes favorece um alto número de escanteios."
                                    else:
                                        return "O histórico recente e o estilo de jogo sugerem uma partida com menos escanteios do que a linha estabelecida."
                                
                                elif market_type == "cards":
                                    if "over" in option:
                                        return "A rivalidade entre as equipes e/ou o histórico disciplinar sugerem um jogo com muitos cartões."
                                    else:
                                        return "O perfil disciplinar das equipes e/ou do árbitro indicam uma partida com poucos cartões."
                                
                                # Justificativa genérica se nenhuma específica for aplicável
                                return f"A análise estatística indica que a probabilidade real de {real_prob:.1f}% para {option_name} oferece valor contra a probabilidade implícita de {implied_prob:.1f}%."

                            def update_opportunities_format(text):
                                """Atualiza formatação para garantir quebras de linhas adequadas em oportunidades."""
                                lines = text.split('\n')
                                formatted_lines = []
                                
                                for line in lines:
                                    # Se a linha for uma justificativa (começa com *Justificativa)
                                    if line.strip().startswith("*Justificativa"):
                                        # Limitar o tamanho da linha com quebras
                                        words = line.split()
                                        new_line = ""
                                        current_length = 0
                                        
                                        for word in words:
                                            if current_length + len(word) > 80:  # Limite de 80 caracteres
                                                new_line += "\n    " + word  # Adicionar quebra de linha com indentação
                                                current_length = 4 + len(word)  # 4 espaços de indentação
                                            else:
                                                if current_length > 0:
                                                    new_line += " " + word
                            ...
                            def reconstruct_analysis(analysis_text, home_team, away_team, selected_markets, original_probabilities, implied_probabilities, odds_data):
                                ...
                                try:
                                    ...
                                    if st.session_state.debug_mode:
                                        st.code(traceback.format_exc())
                                    return "Erro durante a análise. Por favor, tente novamente."
                    
                    except Exception as analysis_error:
                        logger.error(f"Erro durante a análise: {str(analysis_error)}")
                        logger.error(traceback.format_exc())
                        st.error(f"Erro durante a análise: {str(analysis_error)}")
                        if st.session_state.debug_mode:
                            st.code(traceback.format_exc())
        ...
        st.error(f"Erro geral no dashboard: {str(e)}")
        if st.session_state.debug_mode:
            st.code(traceback.format_exc())        
