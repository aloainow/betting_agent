...
const ZeroSpaceComponent = () => {
  useEffect(() => {
    // Este efeito é executado uma vez quando o componente é montado
    // Aplicar os estilos necessários para remover o espaço em branco no topo
    const style = document.createElement('style');
    style.textContent = `
      /* Solução definitiva para remover TODOS os espaços em branco no topo */
      
      /* 1. Reset completo para o container principal */
      .main .block-container {
        padding-top: 0 !important;
        margin-top: 0 !important;
        gap: 0 !important;
      ...
      /* 2. Eliminar completamente o header do Streamlit */
      header[data-testid="stHeader"] {
        display: none !important;
        height: 0 !important;
        min-height: 0 !important;
      ...
      /* 3. Forçar TODOS os elementos iniciais a não terem margem */
      ...
      /* 4. Remover todos os decoradores e widgets de status */
      ...
      /* 5. Remover qualquer padding ou margem do app inteiro */
      .stApp {
        margin: 0 !important;
        padding: 0 !important;
      }
      
      /* 6. Otimizações específicas para mobile */
      @media (max-width: 767px) {
        /* Remover qualquer espaçamento adicional em mobile */
    ...
    // Função que força a remoção de qualquer elemento indesejado
    const forceRemoveElements = () => {
      // Selecionar todos os elementos indesejados
      ...
      // Remover ou ocultar cada elemento
      unwantedElements.forEach(el => {
        if (el.parentNode) {
          // Preferir remover completamente, mas se não for possível, ocultar
          try {
      ...
      // Remover espaçamento do primeiro filho do container principal
      const mainContainer = document.querySelector('.main .block-container');
      if (mainContainer && mainContainer.firstElementChild) {
        mainContainer.firstElementChild.style.marginTop = '0';
        mainContainer.firstElementChild.style.paddingTop = '0';
      }
    };
    ...
    setTimeout(forceRemoveElements, 100);
    setTimeout(forceRemoveElements, 500);
    setTimeout(forceRemoveElements, 1000);
    ...
    // Iniciar observação em todo o corpo do documento
  ...
  return (
    <div className="zero-space-container">
